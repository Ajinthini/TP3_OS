...
/* Structure pour stocker les contacts découverts sur le réseau */
static struct elt *liste = NULL;
static pthread_mutex_t mutex_liste = PTHREAD_MUTEX_INITIALIZER;


/* Identifiants pour les threads de service*/
...
void ajouteElt(char *pseudo, char *adip) {
	...
	/* Vérification pour éviter les doublons dans la liste */
    struct elt *cur = liste;
    while (cur != NULL) {
...
/* Fonctions de recherche pour lier les pseudos aux adresses IP */
...
/* Vérification du format du paquet (Magic number BEUIP) */
...
static void send_broadcast_code(char code, const char *pseudo) {
    ...
    if (inet_aton(BROADCAST_IP, &dst.sin_addr) == 0) {
        return;
    }
...
static void *serveur_udp(void *arg) {
    ...
    /* Timeout sur la réception pour pouvoir vérifier l'état du stop_udp */
    struct timeval tv;
    tv.tv_sec = 1;
    tv.tv_usec = 0;
    ...
    /* Signalement de notre présence au réseau */
